...
- openai-whisper: Installed automatically via `uv tool install openai-whisper`

> **Note**: After installation, if you get `command not found: whisper`, run `uv tool update-shell` to add `~/.local/bin` to your PATH, or manually add it.

> **注意**：安装后如果提示 `command not found: whisper`，请运行 `uv tool update-shell` 将 `~/.local/bin` 添加到 PATH，或手动添加。
